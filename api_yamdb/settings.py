...
DATABASES = {
    "default": {
        ...
        # если написано через get тесты не
        # валятся потому, что туда возвращается none, а при [] исключение
        # которое валит тест. А так этот момент ясен.
...
STATICFILES_DIRS = (os.path.join(BASE_DIR, 'api/redoc'),)
